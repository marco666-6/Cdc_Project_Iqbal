{{-- resources/views/admin/program-magang/create.blade.php --}}
@extends('layouts.admin')

@section('title', 'Tambah Program Magang')
...
<div class="container-fluid">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h1 class="h3 mb-2 fw-bold text-gradient">
                        <i class="bi bi-plus-circle me-2"></i>Tambah Program Magang
                    </h1>
                    <p class="text-muted mb-0">Buat program magang atau MBKM baru</p>
                </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.program-magang.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required>
                                    ...
                                    <option value="mbkm" {{ old('tipe') == 'mbkm' ? 'selected' : '' }}>MBKM</option>
                                    <option value="magang_reguler" {{ old('tipe') == 'magang_reguler' ? 'selected' : '' }}>Magang Reguler</option>
                                    <option value="magang_independen" {{ old('tipe') == 'magang_independen' ? 'selected' : '' }}>Magang Independen</option>
                                </select>
                                @error('tipe')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('benefit') is-invalid @enderror" 
                                      id="benefit" name="benefit" rows="5" required>{{ old('benefit') }}</textarea>
                            @error('benefit')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" class="form-control @error('link_pendaftaran') is-invalid @enderror" 
                                   id="link_pendaftaran" name="link_pendaftaran" value="{{ old('link_pendaftaran') }}" 
                                   placeholder="https://example.com/daftar" required>
...
<script>
    ...
    // Set minimum date for tanggal_berakhir to tomorrow
